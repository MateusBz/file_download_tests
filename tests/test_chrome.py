...
def test_chrome(chrome_driver_init, get_link_locator, test_data):
    index = IndexPage(chrome_driver_init)
    index.navigate(IndexPage.url)
    index.get_resources()
    ebook = KnowlegdeCenterPage(chrome_driver_init)
    url = ebook.get_book_link(get_link_locator)
    ebook.navigate(url)
    login = LoginPage(chrome_driver_init)
